  ...
  for (auto arg : arguments) {
    ...
    if (literalPool.find(literal) == literalPool.end() && arg.type != 2) {
      ...
      if (arg.type == 1) {
        ...
        data = stol(literal, nullptr, base);
      }
      else {
        SymbolTable& symtab = SymbolTable::getInstance();
...
void AssemblyInstructions::st(InstructionArguments* instruction, opcode code) {
  ...
  switch (mod) {
    ...
    case 0x9: {
      ...
      if (arguments[1].type == 0) {
        throw AssemblyException("Error: Unknown value of symbol!");
      }

      if (stoi(*(arguments[1].value)) > MAX_DISPLACEMENT) {
        throw AssemblyException("Error: Maximum displacement exceeded!");
      }
      ...
      int base;
      string literal = *(arguments[1].value);
      if (literal.substr(0, 2) == "0x") {
        base = 16;
      }
      else {
        base = (literal.substr(0, 1) == "0" && literal.length() != 1 ? 8 : 10);
      }
      code.displacement = stoi(literal, nullptr, base);
...
void AssemblyInstructions::ld(InstructionArguments* instruction, opcode code) {
  ...
  switch (mod) {
    ...
    case 0x8: {
      ...
      ss << hex << setw(1) << (LD | mod);
      code.displacement = 0;
      ss << hex << setw(1) << code.first << code.first << code.third << setfill('0') << setw(3) << code.displacement;
      break;
    } 
    ...
    case 0x9: {
      ...
      map<string, int>& literalPool = currentSection->getPool();
      code.displacement = literalPool[*(arguments[1].value)] - currentSection->getLocationCounter() - 4;
      if (code.displacement > MAX_DISPLACEMENT) {
        throw AssemblyException("Error: Maximum displacement exceeded!");
      }
      ...
      if (arguments[1].type == 0) {
        ...
        SymbolTable& symtab = SymbolTable::getInstance();
        int symbolIndex = symtab.getIndexOfEntry(*(arguments[1].value));
        SymbolTableEntry*& stEntry = symtab[symbolIndex];
      ...
      ss.clear();
      ss << hex << setw(1) << (LD | mod);
      code.displacement = 0, code.third = stoi(arguments[0].value->substr(1));
      ss << hex << setw(1) << code.first << code.first << code.third << setfill('0') << setw(3) << code.displacement;
...
void AssemblyInstructions::csrwr(InstructionArguments* instruction) {
  ArgVector arguments = *(instruction->args);
  int mod = instruction->modificator;
  int second = stoi(arguments[1].value->substr(1));
  int first = 0;
